...
int main() {
    try {
        ...
        image8u dilated_and_eroded_mask = morphology::erode(dilated_mask, strength);
        is_foreground_mask = dilated_and_eroded_mask;
        std::cout << "morphology in " << t.elapsed() << " sec" << std::endl;
        debug_io::dump_image("debug/03_is_foreground_dilated.png", dilated_mask);
        debug_io::dump_image("debug/04_is_foreground_dilated_and_eroded.png", dilated_and_eroded_mask);
